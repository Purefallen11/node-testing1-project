...
describe('[Exercise 3] findLargestInteger', () => {
  test('[5] returns the largest number in an array of objects { integer: 2 }', () => {
    const original = [{ integer: 1 }, { integer: 2 }, { integer: 3 }]
    const expected = 3
    
    const actual = utils.findLargestInteger(original)
...
describe('[Exercise 5] Seasons', () => {
  ...
  test('[11] the THIRD call of seasons.next returns "winter"', () => {
    for (let i = 0; i < 2;i++){seasons.next()}
    expect(seasons.next()).toBe('winter')

  })
  test('[12] the FOURTH call of seasons.next returns "spring"', () => {
    for (let i = 0; i < 3;i++){seasons.next()}
...
describe('[Exercise 7] isEvenNumberAsync', () => {
  test('[19] resolves true if passed an even number',async () => {
    let answer = await utils.isEvenNumberAsync(4)
    expect(answer).toBe(true)
  })
  test('[20] resolves false if passed an odd number', async () => {
    let answer = await utils.isEvenNumberAsync(3)
